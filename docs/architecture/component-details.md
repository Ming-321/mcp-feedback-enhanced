...
async def feedback(
    ...
    message: Annotated[str, Field(description="AI 發給用戶的說明或提問內容")] = "我已完成了您請求的任務。",
...
- **工具註冊**: 註冊 `feedback` 和 `get_system_info` 工具
...
**關鍵方法**：
```python
async def create_session(self, project_dir: str, message: str) -> str:
    """創建新會話或更新現有會話"""
...
class WebFeedbackSession:
    ...
    def update_session(self, project_dir: str, message: str, timeout: int):
        """更新會話內容，支援 AI 多次調用"""
```
